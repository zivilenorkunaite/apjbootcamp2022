...
def getOrderItems():
  ...
  ls = []
  juiceSelection = ["Custom", "Juice A", "Juice B", "Juice C", "Juice D"]
  drinkSize = ["Small", "Medium", "Large"]
  allIngredients = ["Apple", "Orange", "Carrot", "Beatroot","Spinach"]
  priceList = [5,7,9]
  for x in range(1, numberOfItems+1):
    item = {"id": random.choice(juiceSelection), "size": random.choice(drinkSize), "notes": random.choices(["","no ice"], weights=[9,1])[0], "cost": 0 }
    item["cost"] = priceList[0] if item["size"] == "Small" else (priceList[1] if item["size"] == "Medium" else priceList[2])
    if item["id"] == "Custom":
      customIngredients = []
      for y in range(1,len(allIngredients)+1):
...
print(getEpoch('2022-01-12', 14 ,41) )
...
def generate_sales_records(sales_date, location, location_timezone, customer_id_list, no_customer_coverage, opening_hours=range(8,18), busy_hours=[4,5,2,2,5,7,4,1,3,1], min_sales = 10, max_sales = 50):
  ...
  df_spec = (dg.DataGenerator(spark, name="single_day_dataset", rows=data_rows,partitions=4)
...
schema = 'array<struct<id:string,size:string,notes:string,cost:double,ingredients:array<string>>>'
...
# MAGIC %sql
# MAGIC select from_json(SaleItems, 'array<struct<id:string,size:string,notes:string,cost:double,ingredients:array<string>>>'), * from tsd_df

# COMMAND ----------

# MAGIC %sql
# MAGIC drop table if exists zivile_demo.apj_locations;
# MAGIC 
# MAGIC create table zivile_demo.apj_locations 
# MAGIC (id string,
...
# MAGIC max_sales int);
# MAGIC 
# MAGIC insert into zivile_demo.apj_locations
# MAGIC values ('AKL01', 'Auckland Airport', '+13:00', '8-17', '1,2,2,5,7,4,2,3,2,1', 50, 0.8, 1000, 5000),
# MAGIC ('AKL02', 'Auckland CBD', '+13:00', '10-17', '2,2,5,7,4,6,6,1', 100, 0.25, 100, 1500),
# MAGIC ('WLG01', 'Wellington CBD', '+13:00', '8-17', '4,5,2,2,5,7,4,1,3,1', 80, 0.3, 100, 2000),
# MAGIC ('SYD01', 'Sydney CBD', '+10:00', '8-19', '4,5,2,2,5,7,4,1,3,5,2,1', 150, 0.5, 2000, 4000),
# MAGIC ('MEL01', 'Melbourne CBD', '+11:00', '8-19', '4,5,2,2,5,7,4,1,3,5,2,1', 150, 0.5, 2000, 4000),
# MAGIC ('MEL02', 'Melbourne Airport', '+11:00', '0-23', '1,1,1,2,1,2,3,6,7,6,3,4,4,2,6,7,4,5,8,8,5,2,1,1', 150, 0.95, 2000, 5000),
# MAGIC ('BNE02', 'Brisbane Airport', '+10:00', '6-19', '2,3,6,7,6,3,4,4,2,6,7,4,5,8', 200, 0.90, 10, 5000)
...
spark.sql("use zivile_demo")
spark.sql("drop table if exists zivile_demo.apj_juice_sales")
records = spark.table("apj_locations").collect()
